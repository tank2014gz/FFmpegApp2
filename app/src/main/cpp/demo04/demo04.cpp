...
const char *className = "com/dr/ffmpeg/jni/FFmpegJNIManager4";
...
jint registerMethod(JNIEnv *env, jint
version) {
...
void unRegisterMethod(JNIEnv *env, jint
version) {
    jclass clazz = env->FindClass(className);
...
JNI_OnLoad(JavaVM
    ...
    if (res == 0) {
        return
                registerMethod(env, JNI_VERSION_1_4
                );
    } else if (vm->GetEnv((void **) &env, JNI_VERSION_1_6) == 0) {
        return
                registerMethod(env, JNI_VERSION_1_6
                );
    }
